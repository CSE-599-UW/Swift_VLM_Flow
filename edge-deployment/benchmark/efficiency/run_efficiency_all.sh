...
# Usage: bash run_all_benchmarks.sh [--num_samples N] [--warmup W] [--max_new_tokens T] [--no_baseline]
# Defaults: num_samples=50, warmup=3, max_new_tokens=256
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --no_baseline)   RUN_BASELINE=false; shift ;;
...
declare -A TRT_ENGINES=(
  ...
  [smoothquant]="$TRT_ENGINE_BASE/qwen2vl_2b_smoothquant"
  [fp8]="$TRT_ENGINE_BASE/qwen2vl_2b_fp8"
  [int4_awq]="$TRT_ENGINE_BASE/qwen2vl_2b_int4_awq"
  [nvfp4]="$TRT_ENGINE_BASE/qwen2vl_2b_nvfp4"
)
# Run order (associative arrays are unordered in bash)
TRT_ORDER=(bf16 int8 int4 smoothquant fp8 int4_awq nvfp4)
...
if $RUN_BASELINE; then
  ...
  echo "  [skipped] baseline (--no_baseline)"
fi
